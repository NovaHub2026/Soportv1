...
| PH-13 | Orbit integration readiness (Owner, 2026-09-14: the broker's frontend `optaqode-frontend2.0` analysed read-only): the integration map, real adapters for identity, records and the staff directory behind the ADR-0002 ports (`optaqode`, environment-selected, fixture-verified), the panel inside the broker, the live connection once the backend answers (BL-001, BL-032) — `PH-13.md` | OBJ-SUP-02; RULE-SUP-01, -05, -07; BL-001 | `ACTIVE` | PH-12; Owner: backend access |
...
## Active chain
Phase: PH-13 `ACTIVE` (`PH-13.md`, Orbit integration readiness — reopened by the Owner on 2026-09-14 after the closure of PH-12): PH-13.1 (integration map and API adapters), PH-13.2 (the panel inside the broker) and PH-13.3 (local integration demo with a simulated broker, `../evidence/PH-13.3-verification.md`) approved; next PH-13.4 (live connection — the Owner creates the service account and grants the dev environment). PH-1..PH-10 and PH-12 `APPROVED`; every cycle audit through cycle 3 and the closing audit are CLOSED. Cycle 4 is at 2/3 (PH-10, PH-12): the next first-time phase approval (PH-11 or PH-13) makes Cycle Audit 4 due. PH-11 (AI assistant first line) stays `PLANNED`: the Owner builds it directly with GPT from `PH-11.md`. Internal demos: `v0.1.0-demo` (superseded), `v0.1.1-demo`, `v0.2.0-demo` (`../evidence/RELEASE-2026-09-14c.md`); local integration demo `node scripts/demo-integration.mjs`. A production release still needs the Owner (§1.1), the live Orbit connection (PH-13.4) and the two open policies (retention, e-mail provider — BL-002). The ledger table below is the only owner of the audit count.

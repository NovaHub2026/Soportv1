...
- Committing (gate-then-commit, FND-0028/FND-0040/FND-0064): write the message to a file and run `bash scripts/gate-commit.sh <message-file> [--no-push] [--include <path>]...` (`npm run gate -- <message-file> …`): it refuses any untracked file not named with `--include`, runs `npm run verify` (output in `.gate-verify.log`, git-ignored), refuses to commit on a non-zero exit, when the tree changed while verify ran, or when a `<NAME>_PLACEHOLDER` token is left, then stages tracked changes plus the named paths, commits and pushes the current branch. Never chain `verify` and `git commit` by hand.
...
- Database: embedded PostgreSQL (PGlite, ADR-0003). No server or Docker needed for development. Dev data lives in `apps/api/.data/pglite` (gitignored; delete to reset). `SUPPORT_DB_DIR` overrides the directory; unset means in memory (tests). A PostgreSQL server is used instead when `SUPPORT_DATABASE_URL` is set (DEC-0032, PH-8.2) — deployments and the `test:pg` runs; `/api/health` says which driver backs the instance. The api suites refuse to run when `SUPPORT_DATABASE_URL` names a database not ending in `_test` (`apps/api/test/database-guard.ts`, FND-0059); `SUPPORT_DB_POOL_MAX` sizes the pool (≥ 2).
...
- Staged attachments (DEC-0037, PH-9.3): `POST /api/support/attachments` (customer, multipart `file`) stores a file before the case exists; `POST /api/support/cases` with `attachmentIds` links it to the first message; it is not downloadable until then.
...
- Test determinism: the api unit and e2e setups save an always-open schedule (00:00–24:00 every day — the decided default since DEC-0039, saved so `workingDefault` is false) so the outside-hours notice never depends on the wall clock; "24:00" is a valid closing time since PH-10.1 (the 23:59 window of FND-0051 is gone).
...
- Hardening (DEC-0031, PH-8.1): the API binds to `127.0.0.1` unless `SUPPORT_BIND` is set (a deployment sets `SUPPORT_BIND=0.0.0.0` behind its proxy and `NODE_ENV=production`); `SUPPORT_SSE_MAX_PER_IDENTITY` (default 8) caps open streams per identity; uploads are limited to 30 per identity per 10 minutes; helmet headers and JSON 404s on every path. Since PH-9.4 (DEC-0038): every 429 carries `Retry-After`; the recovery route limits each client address only when `SUPPORT_TRUST_PROXY` says an appending reverse proxy fronts the web (unset by default; the e2e suite sets 1); uploads never linked to a message are removed after 24 h (`SUPPORT_ATTACHMENT_CLEANUP_JOB`, `SUPPORT_ATTACHMENT_CLEANUP_INTERVAL_MS`, `SUPPORT_UNLINKED_ATTACHMENT_GRACE_HOURS`).
- Sign-out (DEC-0030, PH-7.3): "Sair" on `/` and `/staff` clears the simulated selection (`localStorage`) and the app's `sessionStorage` buffers; the host signs out after 30 min idle (`IDLE_SIGN_OUT_MS` in `apps/web/src/lib/use-idle-sign-out.ts`).
- Roles (DEC-0029, PH-7.2): staff resolve only from the simulated directory (`SIMULATED_STAFF_DIRECTORY`: ana and bruno agents, carla supervisor); `x-simulated-staff-role` cannot promote anyone — the directory's role wins. Agents get 403 `not_case_owner` on state changes of a colleague's case. Formal complaints (DEC-0041, PH-10.2): category `formal_complaint`; an agent gets 403 `supervisor_required` on any staff action on one, and handing one to an agent is 400; `GET /api/staff/overview` carries `complaints { count, overdue, list }`.
...
- Access recovery (DEC-0028, PH-7.1): `POST /api/public/access-recovery` needs no identity (`{ contact, description, clientRequestId? }` → `REC-…` receipt; 429 after 3 per contact per hour, 10 per client address (behind an appending proxy — `SUPPORT_TRUST_PROXY`) or 120 per instance per 10 minutes (every 429 carries `Retry-After` — DEC-0038)); staff: `GET /api/staff/access-recovery?status=received|forwarded|closed`, `POST /api/staff/access-recovery/<id>/handle { outcome: forwarded|closed, note? }`. The hand-off to Orbit is simulated (labeled).
...
| gate | `bash scripts/gate-commit.sh <message-file> [--include <path>]...` | untracked check → verify → unchanged-tree check → PLACEHOLDER check → stage named paths → commit → push | Every commit |
| api on PostgreSQL | `docker compose -f docker/compose.test.yml up -d` then `SUPPORT_DATABASE_URL=postgres://postgres:orbit@localhost:55433/orbit_test npm run test:pg -w api` | Unit + e2e suites against PostgreSQL 16 with a real pool (BL-019); `apps/api/scripts/pg-reset.mjs` empties only databases named `*_test` | Release candidate; CI runs it on every push (`verify-postgres`) |
...
## CI
`.github/workflows/ci.yml` runs two jobs on every push to `main` and on pull requests: `verify` (`npm ci && npm run verify && npm run build`) and `api suites on PostgreSQL 16` (`npm run test:pg -w api` against a service container — PH-8.2). CI runs after integration to `main`, so a push is "awaiting corroboration" until the run completes (§9.2). The `verify` job checks out full history (`fetch-depth: 0`): `check-context` verifies the commits and tags live documents cite (BL-030). Check with `gh run list --limit 3` / `gh run view <id>`; record the verdict per candidate in evidence.
...
- The UI smoke is not in CI (DEC-0027 e; BL-008 revisit at the first hosted deployment).

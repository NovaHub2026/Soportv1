...
- Reminders and outside hours (PH-6.3): `SUPPORT_REMINDER_JOB=off` disables the reminder job, `SUPPORT_REMINDER_INTERVAL_MS` sets its cadence (default 60 000); the outside-hours notice follows the configured schedule (close every day in the supervision settings to see it).

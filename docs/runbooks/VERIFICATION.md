...
- Orbit records (DEC-0019, PH-4): `SUPPORT_ORBIT_RECORDS=simulated` is the only adapter; `SUPPORT_SIMULATED_ORBIT=unavailable` makes every lookup answer "unavailable" to exercise that experience. `GET /api/staff/cases/<id>/orbit` returns the customer's masked summary or the unavailable state; `/api/health` reports `orbitRecords`.

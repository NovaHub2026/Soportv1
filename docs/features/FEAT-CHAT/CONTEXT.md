...
Lifecycle: PARTIAL
Freshness: CURRENT
Verified against: `c3fc6a8` plus the PH-5.4 change (availability line from the configured schedule)
...
Notifications (PH-6.1, FEAT-NOTIFY): the host's "Notificações" button carries the unread count and opens the panel on the notified case; opening a conversation marks its notifications read (`POST /support/cases/:id/read`).

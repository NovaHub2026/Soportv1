...
Availability (PH-5.4, RULE-SUP-08; PH-10.1, DEC-0039 a, DEC-0040): the home shows "Atendimento 24 horas, todos os dias." while the decided 24/7 schedule stands; when a supervisor narrows it, "Atendimento aberto/fechado agora", today's window or "Hoje não há atendimento" and the next opening — formatted from the instants of `GET /api/support/availability` (`todayWindow`, `nextOpeningAt`) in the browser's zone, always with "Horários no seu fuso (<zone>)" (`localOpening`, `customerTimeZone` in `apps/web/src/i18n/index.ts`); no response-time promise anywhere.

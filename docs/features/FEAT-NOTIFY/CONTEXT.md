...
Delivery guarantee (DEC-0027 b, DEC-0034 h, FND-0076): e-mail is at most once — a crash between claiming a row and sending it loses that e-mail (the in-product notification survives); a send that keeps failing is retried every tick without a cap until BL-028. Since PH-12.1 (DEC-0043 b) a failing send counts an attempt (`email_attempts`) and after `SUPPORT_EMAIL_MAX_ATTEMPTS` (5) the row is a dead letter (`email_failed_at`): logged as given up, never retried, the in-product notification untouched. Since the closing audit (FND-0109, DEC-0044 d) the claim also requires `email_failed_at IS NULL`, the attempt count and the parking decision come from one `UPDATE … RETURNING` on the stored column (never from the row read before the send), and the log line carries the error's name and a bounded message — never the provider's stack, which could quote the recipient. Re-queuing a dead letter: `../../runbooks/OPERATIONS.md`.
...
DEC-0024 (notification kinds and transactional recording), DEC-0025 (e-mail channel), DEC-0026 (automated notices), DEC-0027 (reminder period, claim-then-send, time-zone validation), DEC-0040 (instants and `nextOpeningAt`), DEC-0043 b and DEC-0044 d (dead letters). Assumption: the host hosts the bell; when Orbit hosts the panel for real, the same endpoints feed Orbit's own notification surface.

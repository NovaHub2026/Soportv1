...
Cycle 3 out-of-band audit (DEC-0034 g): contacts are normalized (NFKC, no invisible characters, e-mail without "+tag" or trailing dot, phone as digits) for the per-contact limit and idempotency; contact and text refuse control and bidi characters; the instance-wide ceiling (120 per 10 minutes) answers 429 `service_busy` and the page says the service is busy instead of blaming the person's contact; the staff page reminds never to ask for secrets. Carried: `Retry-After` and a per-client limit (BL-026), masking pasted secrets (BL-027). Sign-out: the server render and the first client render are identity-neutral (FND-0060); a sign-out in another tab clears this tab's drafts; the idle timer counts scrolls inside the panel and shares the latest activity across tabs.

...
Lifecycle: PARTIAL
Freshness: CURRENT
Verified against: `11f178a` plus the Cycle Audit 1 remediation (shared staff directory, consultation guard message, role labels)
...
Component change → `npm test -w web`, `npm run lint -w web`; layout/copy change → `npm run build` + `scripts/ui-smoke.mjs`. Last scoped evidence: `docs/evidence/PH-3.5-verification.md`, `docs/evidence/CYCLE-1-verification.md`.

...
Lifecycle: PARTIAL
Freshness: CURRENT
Verified against: `3fc99b0` plus the PH-3.2 change (note mode, consultations)
...
Gaps (accepted target): transfer/release/reassignment, priority/category edit (PH-3.3); filters, search by reference / user id / email / operation refs, saved replies, supervision views, schedule/config, metrics (PH-5); Orbit record cards and masked identity summary (PH-4); role-based permissions beyond agent/supervisor/admin labels (PH-7); the context column is hidden below 1100 px (desktop-first).

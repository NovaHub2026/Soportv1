import type {
  CaseCategory,
  CaseEventType,
  CasePriority,
  CaseStatus,
  ConsultationTeam,
  NotificationKind,
  OrbitAccountStatus,
  OrbitEnvironment,
  OrbitRecordKind,
  OrbitUnavailableReason,
  OrbitVerificationStatus,
  ResolutionReason,
  StaffQueueView,
  StaffRole,
  Weekday,
} from "@orbit-support/shared";

/** Customer-facing copy, Brazilian Portuguese first (PROJECT_CONTEXT.md §10.1). */
export const ptBR = {
  locale: "pt-BR",
  units: {
    minutes: "min",
    hours: "h",
    days: "d",
  },
  app: {
    title: "Orbit — Suporte",
    description: "Atendimento ao cliente Orbit: fale com o suporte e acompanhe seus pedidos.",
    simulationBadge: "Simulação",
    simulationNote:
      "Ambiente de desenvolvimento: a identidade e os dados do Orbit são simulados. Nada aqui é uma conta real.",
  },
  access: {
    link: "Não consigo acessar minha conta",
    title: "Não consigo acessar minha conta",
    intro: "Deixe um contato e conte o que está acontecendo. Uma pessoa da equipe vai retornar por esse contato para orientar a recuperação pelo processo de verificação do Orbit.",
    noSecrets: "Nunca pedimos sua senha, código de verificação (2FA) ou frase de recuperação — nem aqui, nem por e-mail ou telefone. Nenhum dado da conta é mostrado antes da verificação.",
    contact: "E-mail ou telefone para contato",
    contactHint: "Não precisa ser o cadastrado no Orbit; é só para a equipe falar com você.",
    description: "O que está acontecendo?",
    descriptionHint: "Por exemplo: o código de verificação não chega; perdi o acesso ao e-mail; a conta parece bloqueada.",
    submit: "Enviar pedido",
    sending: "Enviando…",
    cancel: "Cancelar",
    receiptTitle: "Pedido registrado",
    receivedAt: "Recebido às {time}.",
    nextStep: "Próximo passo: a equipe de Verificação entra em contato pelo canal informado e conduz a recuperação pelo processo de verificação do Orbit. Guarde a referência acima para acompanhar.",
    back: "Voltar",
    tooMany: "Recebemos vários pedidos deste contato. Aguarde cerca de {minutes} min antes de tentar de novo.",
    busy: "Recebemos muitos pedidos no momento. Tente de novo em cerca de {minutes} min.",
    fromClient: "Recebemos muitos pedidos desta conexão. Tente de novo em cerca de {minutes} min.",
    rules: "Para enviar: um e-mail ou telefone válido e uma descrição com pelo menos 10 caracteres.",
    failed: "Não foi possível enviar o pedido. Verifique sua conexão e tente novamente.",
    simulation: "Nesta versão o encaminhamento ao processo de verificação do Orbit é simulado: nada é enviado ao Orbit.",
  },
  shell: {
    brand: "Orbit",
    tradingPlaceholder: "Área de negociação",
    tradingPlaceholderHint: "Espaço reservado para a interface do Orbit. O suporte fica ao lado, sem cobrir os controles de operação.",
    openSupport: "Suporte",
    closeSupport: "Fechar suporte",
    account: "Conta simulada",
    loading: "Carregando…",
    signOut: "Sair",
    signedOut: {
      title: "Quem está usando este dispositivo?",
      hint: "Escolha a conta simulada para continuar. Nada da pessoa anterior fica visível ou guardado neste navegador.",
      done: "Você saiu. Este dispositivo não mostra mais suas conversas.",
      idle: "Sua sessão foi encerrada por inatividade.",
      enter: "Entrar",
    },
  },
  embed: {
    waiting: "Conectando ao suporte…",
    tokenRequired: "Sua sessão precisa ser renovada. Aguarde um instante ou entre de novo no Orbit.",
    refused: "O suporte não pode ser aberto a partir desta página.",
  },
  support: {
    title: "Suporte",
    back: "Voltar",
    home: {
      talk: "Falar com o suporte",
      availability:
        "Atendimento feito por pessoas. Sua mensagem fica registrada com um número de referência e a resposta aparece aqui — mesmo se você fechar o painel.",
      openNow: "Atendimento aberto agora.",
      closedNow: "Atendimento fechado agora.",
      todayHours: "Hoje: {open}–{close}.",
      todayAllDay: "Hoje: atendimento 24 horas.",
      dayHours: "{day}: {open}–{close}.",
      openUntil: "Atendimento 24 horas até {day} às {time}.",
      closedToday: "Hoje não há atendimento.",
      nextOpening: "Próximo atendimento: {day} às {time}.",
      alwaysOpen: "Atendimento 24 horas, todos os dias.",
      yourZone: "Horários no seu fuso ({tz}).",
      weekdays: {
        mon: "segunda",
        tue: "terça",
        wed: "quarta",
        thu: "quinta",
        fri: "sexta",
        sat: "sábado",
        sun: "domingo",
      } satisfies Record<Weekday, string>,
      active: "Conversas em andamento",
      previous: "Conversas anteriores",
      empty: "Você ainda não falou com o suporte.",
      loading: "Carregando suas conversas…",
      error: "Não foi possível carregar suas conversas.",
      retry: "Tentar novamente",
    },
    newRequest: {
      title: "Como podemos ajudar?",
      topic: "Assunto",
      message: "Conte o que está acontecendo",
      messageHint: "Não precisa informar seu ID ou e-mail: já sabemos quem você é.",
      send: "Enviar",
      sending: "Enviando…",
      error: "Não foi possível enviar. Verifique sua conexão e tente novamente.",
      attachmentGone: "Um dos arquivos anexados não está mais disponível. Anexe de novo e envie.",
      validation: "Escolha um assunto e escreva sua mensagem.",
      complaintHint: "Uma reclamação formal é registrada com número próprio e tratada por um supervisor, ",
    },
    conversation: {
      reference: "Referência",
      you: "Você",
      support: "Suporte",
      system: "Aviso",
      composerLabel: "Sua mensagem",
      composerPlaceholder: "Escreva sua mensagem",
      send: "Enviar",
      sending: "Enviando…",
      failed: "Não enviada",
      retry: "Reenviar",
      refused: "Recusada pelo servidor",
      discard: "Descartar",
      movedToFollowUp: "O caso foi encerrado antes de a sua última mensagem chegar. Ela está pronta abaixo para a continuação.",
      loading: "Carregando conversa…",
      error: "Não foi possível carregar a conversa.",
      closedNotice: "Esta conversa foi encerrada. Se ainda precisar de ajuda sobre este assunto, conte o que está acontecendo: abriremos uma continuação ligada a este caso, com um novo número.",
      followUpLabel: "O que ainda precisa",
      followUpPlaceholder: "Descreva o que ainda está acontecendo",
      followUpSubmit: "Preciso de mais ajuda",
      followUpSending: "Abrindo continuação…",
      followUpFailed: "Não foi possível abrir a continuação. Tente novamente.",
      followUpOf: "Continuação do caso {reference}",
      openPrevious: "Ver caso anterior",
      waitingNotice: "Recebemos sua mensagem. Uma pessoa da equipe vai responder por aqui.",
      unreadOne: "1 nova mensagem",
      unreadMany: "{n} novas mensagens",
      resolvedNotice: "Este caso foi marcado como resolvido. Se ainda precisar de ajuda, é só responder ou tocar no botão abaixo.",
      stillNeedHelp: "Ainda preciso de ajuda",
      stillNeedHelpMessage: "Ainda preciso de ajuda.",
    },
    connection: {
      connecting: "Conectando…",
      connected: "Ao vivo",
      reconnecting: "Reconectando…",
      closed: "Sem conexão",
    },
    emails: {
      preference: "Receber e-mail quando houver resposta e eu não tiver visto",
      preferenceHint: "O e-mail traz só a referência do caso e um link para a conversa; nunca o conteúdo.",
      outboxTitle: "E-mails que seriam enviados",
      outboxHint: "Simulação: nenhum e-mail real é enviado nesta versão. Esta lista mostra o que o serviço de e-mail receberia.",
      outboxEmpty: "Nenhum e-mail ainda.",
      outboxError: "Não foi possível carregar a lista de e-mails.",
      preferenceUnavailable: "A preferência de e-mail não pôde ser carregada agora.",
      preferenceFailed: "Não foi possível salvar a preferência. O valor anterior continua valendo.",
      to: "para {to}",
      open: "Abrir conversa",
    },
    notifications: {
      label: "Notificações",
      unreadLabel: "Notificações: {n} não lidas",
      empty: "Nenhuma notificação por enquanto.",
      failed: "Não foi possível carregar as notificações.",
      retry: "Tentar de novo",
      kinds: {
        staff_reply: "Nova resposta em {reference}",
        waiting_customer: "Precisamos da sua resposta em {reference}",
        resolved: "{reference} foi marcado como resolvido",
        closed: "{reference} foi encerrado",
        reminder: "Lembrete: {reference} aguarda sua resposta",
        outside_hours: "Fora do horário: {reference} foi registrado",
      } satisfies Record<NotificationKind, string>,
    },
    records: {
      title: "Seus registros",
      hint: "Dados simulados: o Orbit ainda não está conectado. Toque em “Preciso de ajuda” para falar com o suporte sobre um registro.",
      loading: "Carregando registros…",
      unavailable: "Registros indisponíveis no momento.",
      empty: "Nenhum registro recente.",
      help: "Preciso de ajuda",
      about: "Sobre este registro",
      remove: "Remover registro",
      removedHint: "A pergunta será geral, sem registro anexado.",
      activeCase: "Este registro já tem uma conversa em andamento ({reference}).",
      continueCase: "Continuar conversa",
      differentIssue: "É outro problema",
      snapshotAt: "dados de {time}",
      notFound: "Registro não encontrado no Orbit quando o caso foi aberto. A equipe vai investigar.",
      unavailableAtOpen: "O Orbit não respondeu quando o caso foi aberto. A equipe vai investigar.",
      kinds: {
        operation: "Operação",
        pix_deposit: "Depósito Pix",
        withdrawal: "Saque",
      } satisfies Record<OrbitRecordKind, string>,
    },
  },
  /** System messages, worded from their kind (PH-9.2, BL-024); the API keeps the text as written at the time. */
  systemMessages: {
    followUpOf: "Continuação do caso {reference}.",
    outsideHours: "Fora do horário de atendimento. Registramos sua mensagem; ela será atendida por uma pessoa.",
  },
  attachments: {
    attach: "Anexar arquivo",
    hint: "PNG, JPEG, WebP ou PDF · até 10 MB · máximo de 3 por mensagem",
    uploading: "Enviando arquivo…",
    uploadFailed: "Não foi possível enviar o arquivo.",
    tooLarge: "Arquivo maior que 10 MB.",
    unsupported: "Tipo não permitido. Envie PNG, JPEG, WebP ou PDF.",
    tooMany: "Máximo de 3 arquivos por mensagem.",
    remove: "Remover",
    open: "Abrir",
    loading: "Carregando arquivo…",
    unavailable: "Arquivo indisponível",
    imageAlt: "Imagem anexada: {name}",
    pendingFiles: "Anexos: {names}",
  },
  staff: {
    title: "Orbit — Área de suporte",
    workspace: "Área de suporte",
    agentPicker: "Atendente simulado",
    queuesLabel: "Filas de casos",
    signOut: "Sair",
    signedOut: {
      title: "Quem está usando esta estação?",
      hint: "Escolha o atendente simulado para continuar. A fila e os casos do atendente anterior não ficam nesta tela.",
      enter: "Entrar",
    },
    queues: {
      unassigned: "Não atribuídos",
      mine: "Meus casos",
      active: "Todos ativos",
      waiting_customer: "Aguardando cliente",
      waiting_internal: "Aguardando equipe",
      resolved: "Resolvidos",
      closed: "Encerrados",
    } satisfies Record<StaffQueueView, string>,
    queueEmpty: {
      unassigned: "Nenhum caso aguardando atribuição.",
      mine: "Você não tem casos ativos.",
      active: "Nenhum caso ativo no momento.",
      waiting_customer: "Nenhum caso aguardando o cliente.",
      waiting_internal: "Nenhum caso aguardando outra equipe.",
      resolved: "Nenhum caso resolvido.",
      closed: "Nenhum caso encerrado.",
    } satisfies Record<StaffQueueView, string>,
    supervision: {
      open: "Supervisão",
      title: "Supervisão",
      demand: "Demanda em aberto",
      unassigned: "Sem responsável",
      awaitingReply: "Aguardando resposta humana",
      waitingInternal: "Aguardando equipe interna",
      oldest: "mais antigo há {age}",
      byAgent: "Carga por atendente",
      noAgents: "Nenhum caso atribuído.",
      agentLoad: "{open} em aberto · {awaiting} sem resposta",
      overdue: "Acompanhamento atrasado (mais de {hours} h sem resposta)",
      noOverdue: "Nenhum caso atrasado.",
      reassignTo: "Reatribuir a",
      reassigned: "Caso reatribuído.",
      reassignPlaceholder: "Escolher agente…",
      invalid: "Configuração inválida ({fields}). Nada foi salvo.",
      metrics: "Resultados do serviço",
      period: "Período",
      days: "{n} dias",
      metricsNote: "Calculado a partir do histórico do período. Não há metas definidas pela operação; estes números descrevem o que aconteceu.",
      created: "Casos abertos",
      resolved: "Resoluções",
      closed: "Encerramentos",
      reopened: "Reaberturas",
      firstResponse: "Primeira resposta humana",
      resolution: "Tempo até a resolução",
      unansweredNow: "Sem resposta agora",
      stats: "mediana {median} · p90 {p90} · n={n}",
      noData: "sem dados",
      settings: "Configuração do atendimento",
      workingDefault: "Valores definidos pela operação (DEC-0039); nenhum supervisor os alterou ainda.",
      allDay: "Dia inteiro",
      allDayOn: "Dia inteiro em {day}",
      configuredBy: "Configurado por {agent} · {time}.",
      timezone: "Fuso horário (IANA)",
      openOn: "Atende em {day}",
      opensAt: "Abre em {day}",
      closesAt: "Fecha em {day}",
      threshold: "Horas sem resposta para considerar atraso",
      followUpWindow: "Dias para encerrar um caso resolvido",
      emailDelay: "Minutos sem ler uma notificação antes de enviar e-mail (0 = imediato)",
      reminderAfter: "Horas aguardando o cliente antes de um lembrete",
      saveSettings: "Salvar configuração",
      settingsSaved: "Configuração salva.",
      forbidden: "Só supervisores podem ver ou alterar isto.",
      failed: "Não foi possível carregar ou salvar.",
      exports: {
        title: "Exportar dados de um cliente",
        hint: "Só administradores. A exportação contém o que o cliente pode ver (casos, mensagens públicas, anexos, histórico e preferências) e fica registrada com quem pediu e por quê (DEC-0039).",
        customerId: "ID Orbit do cliente",
        reason: "Motivo (pedido do cliente)",
        submit: "Exportar",
        exporting: "Exportando…",
        done: "Exportação {id} registrada: {count} caso(s) de {customer}. O download foi iniciado.",
        downloadAgain: "Baixar de novo",
        invalid: "Não foi possível exportar. Verifique o ID e o motivo.",
        failed: "Não foi possível exportar agora. Tente de novo.",
        listFailed: "Não foi possível carregar as exportações registradas.",
        forbidden: "Só administradores podem exportar dados de clientes.",
        recent: "Exportações registradas",
        none: "Nenhuma exportação registrada.",
        recordLine: "{time} · {customer} · {count} caso(s) · por {agent} · {reason}",
      },
      weekdays: {
        mon: "Segunda",
        tue: "Terça",
        wed: "Quarta",
        thu: "Quinta",
        fri: "Sexta",
        sat: "Sábado",
        sun: "Domingo",
      } satisfies Record<Weekday, string>,
    },
    accessRecovery: {
      open: "Recuperação de acesso",
      title: "Recuperação de acesso",
      intro: "Pedidos de quem não consegue entrar: um contato não verificado e uma descrição. Nenhum pedido está ligado a uma conta — retorne pelo contato informado e encaminhe à equipe de Verificação, que conduz a recuperação pelo processo KYC do Orbit.",
      noSecrets: "Nunca peça senha, código de verificação (2FA) ou frase de recuperação. Se um pedido contiver algum desses dados, não o use nem o repita; oriente pelo processo de verificação do Orbit.",
      filter: "Mostrar",
      all: "Todos",
      statuses: { received: "Recebido", forwarded: "Encaminhado à Verificação", closed: "Encerrado" },
      contact: "Contato",
      note: "Observação (opcional)",
      forward: "Encaminhar à equipe de Verificação",
      close: "Encerrar",
      forwardHint: "A equipe de Verificação conduz a recuperação pelo processo KYC do Orbit (DEC-0039). O encaminhamento é simulado nesta versão: fica registrado quem encaminhou e quando.",
      forwardedDone: "{reference} encaminhado à equipe de Verificação (simulação).",
      closedDone: "{reference} encerrado.",
      handledBy: "Tratado por {agent} em {time}",
      masked: "Trecho oculto: parecia uma senha ou um código. O texto original fica apenas com o processo de verificação.",
      alreadyHandled: "Este pedido já foi tratado.",
      failed: "Não foi possível registrar o desfecho.",
      loadFailed: "Não foi possível carregar os pedidos.",
      empty: "Nenhum pedido neste filtro.",
    },
    savedReplies: {
      title: "Respostas salvas",
      open: "Respostas salvas",
      back: "Voltar aos casos",
      hint: "Modelos mantidos pela equipe. Ao inserir uma resposta, ajuste o texto antes de enviar; nada é enviado automaticamente.",
      create: "Nova resposta",
      edit: "Editar",
      remove: "Remover",
      cancel: "Cancelar",
      save: "Salvar",
      titleField: "Título",
      bodyField: "Texto",
      categoryField: "Assunto (opcional)",
      anyCategory: "Qualquer assunto",
      empty: "Nenhuma resposta salva ainda.",
      loadFailed: "Não foi possível carregar as respostas salvas.",
      confirmRemove: "Confirmar remoção",
      failed: "Não foi possível salvar a resposta.",
      forbidden: "Só quem criou a resposta, ou um supervisor, pode alterá-la.",
      updatedBy: "atualizada por {agent} · {time}",
      picker: "Inserir resposta salva",
      pickerNone: "— escolher —",
    },
    search: {
      label: "Buscar casos",
      placeholder: "Referência, ID do cliente, assunto ou registro",
      category: "Filtrar por assunto",
      priority: "Filtrar por prioridade",
      agent: "Filtrar por responsável",
      anyCategory: "Todos os assuntos",
      anyPriority: "Todas as prioridades",
      anyAgent: "Qualquer responsável",
      clear: "Limpar",
    },
    awaitingReply: "Sem resposta há {age}",
    waitingCustomerSince: "Aguardando o cliente há {age}",
    waitingInternalSince: "Aguardando a equipe há {age}",
    loadMore: "Carregar mais",
    listCapped: "A lista mostra no máximo {n} casos. Use a busca ou os filtros para restringir a lista.",
    queueStale: "A lista pode estar desatualizada: a última atualização falhou.",
    loading: "Carregando…",
    error: "Não foi possível carregar.",
    retry: "Tentar novamente",
    selectCase: "Selecione um caso na fila para ver a conversa.",
    take: "Assumir caso",
    taking: "Assumindo…",
    takeFailed: "Não foi possível assumir o caso. Ele pode já ter um responsável.",
    responsible: "Responsável",
    unassigned: "Sem responsável",
    you: "você",
    reply: "Responder ao cliente",
    replyLabel: "Resposta ao cliente",
    replyPlaceholder: "Escreva a resposta que o cliente vai ver…",
    sending: "Enviando…",
    sendFailed: "Não foi possível enviar a resposta. Tente novamente.",
    internalNote: "Nota interna",
    internalNoteHint: "visível só para a equipe",
    customer: "Cliente",
    system: "Sistema",
    unreadOne: "1 nova do cliente",
    unreadMany: "{n} novas do cliente",
    customerRead: "Última resposta lida pelo cliente",
    customerUnread: "Última resposta ainda não lida pelo cliente",
    connection: {
      connecting: "Conectando…",
      connected: "Ao vivo",
      reconnecting: "Reconectando…",
      closed: "Sem conexão",
    },
    context: {
      title: "Contexto",
      customer: "Cliente",
      customerId: "ID Orbit",
      identitySource: "Identidade",
      simulated: "Simulada — o Orbit ainda não existe",
      caseSection: "Caso",
      category: "Assunto",
      priority: "Prioridade",
      createdAt: "Aberto em",
      lastCustomerMessage: "Última mensagem do cliente",
      lastStaffMessage: "Última resposta da equipe",
      none: "—",
      history: "Histórico",
    },
    actions: {
      title: "Ações do caso",
      waitCustomer: "Aguardar cliente",
      waitInternal: "Aguardar equipe interna",
      resume: "Retomar atendimento",
      resolve: "Resolver caso",
      resolveReason: "Motivo",
      resolveExplanation: "Explicação para o cliente",
      resolveExplanationHint: "O cliente verá este texto na conversa, junto com o status “Resolvido”.",
      resolveConfirm: "Confirmar resolução",
      cancel: "Cancelar",
      failed: "Não foi possível aplicar a ação. Tente novamente.",
      consultationsOpen: "Há consulta pendente com outra equipe. Registre a resposta antes de resolver o caso.",
      notOwner: "Só o responsável ou um supervisor pode alterar o status, resolver, encerrar, consultar equipe ou editar este caso. Você pode responder ao cliente e registrar notas internas.",
      resolvedAs: "Resolvido · {reason}",
      close: "Encerrar caso",
      followUpOf: "Continuação do caso {reference}",
      openPrevious: "Abrir caso anterior",
    },
    closedReasons: {
      auto_window: "após o prazo de acompanhamento",
      staff: "pela equipe",
    },
    complaint: {
      tag: "Reclamação formal",
      deadline: "Prazo de resposta: {when}",
      overdue: "Prazo vencido há {age}",
      deadlinePast: "Prazo era {when}",
      supervisorOnly: "Reclamações formais são tratadas por supervisores. Um agente não pode assumir, responder nem alterar este caso.",
      section: "Reclamações formais (prazo de 5 dias úteis)",
      none: "Nenhuma reclamação formal em aberto.",
      summary: "{count} em aberto · {overdue} com prazo vencido",
    },
    incidents: {
      title: "Incidente compartilhado",
      tag: "Incidente",
      none: "Este caso não está vinculado a um incidente.",
      link: "Vincular a incidente",
      existing: "Incidente existente",
      choose: "Escolha um incidente aberto",
      confirmLink: "Vincular",
      create: "Criar incidente",
      newTitle: "Título do incidente",
      newTitlePlaceholder: "Ex.: Atraso no provedor Pix",
      confirmCreate: "Criar e vincular",
      cancel: "Cancelar",
      unlink: "Desvincular",
      broadcastLabel: "Nota interna para todos os casos vinculados",
      broadcastSubmit: "Enviar nota a todos",
      delivered: "Nota enviada a {n} caso(s) vinculado(s).",
      resolve: "Marcar incidente como resolvido",
      resolveHint: "Marcar o incidente não resolve os casos: cada cliente precisa de confirmação individual.",
      statusOpen: "Em andamento",
      statusResolved: "Resolvido",
      linkedCount: "{n} caso(s) vinculado(s)",
      failed: "Não foi possível atualizar o incidente.",
      linkedEvent: "Vinculado ao incidente “{incident}”",
      unlinkedEvent: "Desvinculado do incidente",
    },
    assignment: {
      transfer: "Transferir",
      transferTo: "Transferir para",
      confirm: "Confirmar transferência",
      release: "Devolver à fila",
      cancel: "Cancelar",
      forbidden: "Só o responsável ou um supervisor pode transferir este caso.",
      failed: "Não foi possível alterar o responsável.",
      transferredEvent: "Transferido para {agent} por {by}",
      releasedEvent: "Devolvido à fila por {by}",
    },
    attributes: {
      priority: "Prioridade",
      category: "Assunto",
      failed: "Não foi possível atualizar o caso.",
    },
    notes: {
      modeLabel: "Tipo de mensagem",
      replyMode: "Responder ao cliente",
      noteMode: "Nota interna",
      noteLabel: "Nota interna (só a equipe vê)",
      notePlaceholder: "Escreva uma nota para a equipe…",
      save: "Salvar nota",
      saving: "Salvando…",
      failed: "Não foi possível salvar a nota.",
    },
    consultations: {
      title: "Consultas a outras equipes",
      request: "Consultar equipe",
      team: "Equipe",
      question: "Pergunta para a equipe",
      submit: "Enviar consulta",
      cancel: "Cancelar",
      pending: "Aguardando resposta",
      answered: "Respondida",
      answerLabel: "Resposta da equipe",
      answerSubmit: "Responder consulta",
      requestedBy: "Pedida por {agent} · {time}",
      answeredBy: "Respondida por {agent} · {time}",
      none: "Nenhuma consulta neste caso.",
      openOne: "1 consulta pendente",
      openMany: "{n} consultas pendentes",
      failed: "Não foi possível registrar a consulta.",
    },
    orbit: {
      title: "Cliente no Orbit",
      loading: "Consultando o Orbit…",
      unavailable: "Dados do Orbit indisponíveis: {reason}.",
      retry: "Tentar novamente",
      reasons: {
        not_integrated: "integração ainda não construída",
        not_found: "cliente não encontrado no Orbit",
        unavailable: "Orbit sem resposta",
        timeout: "tempo de resposta esgotado",
      } satisfies Record<OrbitUnavailableReason, string>,
      username: "Usuário",
      accountStatus: "Conta",
      environment: "Ambiente",
      verification: "Verificação",
      language: "Idioma · país",
      registeredAt: "Cadastro",
      email: "E-mail",
      phone: "Telefone",
      fetchedAt: "Consultado",
      accountStatuses: {
        active: "Ativa",
        restricted: "Restrita",
        blocked: "Bloqueada",
        closed: "Encerrada",
      } satisfies Record<OrbitAccountStatus, string>,
      verificationStatuses: {
        verified: "Verificada",
        pending: "Em análise",
        unverified: "Não verificada",
        rejected: "Recusada",
      } satisfies Record<OrbitVerificationStatus, string>,
      environments: {
        real: "Conta real",
        demo: "Conta demo",
      } satisfies Record<OrbitEnvironment, string>,
      notIntegratedTitle: "Ainda não integrado",
      notIntegratedHint: "Estes assuntos ainda não chegam pelo Orbit; a equipe deve tratá-los como informação indisponível, não como zero.",
      notIntegrated: ["Saldos e movimentos", "Bônus e promoções", "Eventos P2P", "Indicações e afiliados", "Erros do produto"],
      record: {
        title: "Registro vinculado",
        current: "Estado atual no Orbit",
        unavailable: "Estado atual indisponível: {reason}.",
      },
    },
    roles: {
      agent: "Atendente",
      supervisor: "Supervisão",
      admin: "Administração",
    } satisfies Record<StaffRole, string>,
    teams: {
      finance: "Financeiro",
      operations: "Operações",
      security: "Segurança",
      verification: "Verificação",
      product: "Produto",
    } satisfies Record<ConsultationTeam, string>,
    reasons: {
      solved: "Problema resolvido",
      answered: "Dúvida respondida",
      no_action_possible: "Analisado, sem ação possível",
      handled_elsewhere: "Encaminhado ao processo responsável",
      duplicate: "Duplicado de outro caso",
      no_customer_response: "Sem resposta do cliente",
    } satisfies Record<ResolutionReason, string>,
    events: {
      case_created: "Caso aberto pelo cliente",
      case_assigned: "Atribuído a {agent}",
      status_changed: "Status: {from} → {to}",
      case_reopened: "Reaberto pelo cliente",
      case_resolved: "Resolvido: {reason}",
      case_closed: "Encerrado {how}",
      priority_changed: "Prioridade: {from} → {to}",
      category_changed: "Assunto: {from} → {to}",
      consultation_requested: "Consulta enviada para {team}",
      consultation_answered: "Consulta respondida por {agent}",
      reminder_sent: "Lembrete enviado ao cliente (automático)",
      follow_up_created: "Continuação aberta: {reference}",
      incident_linked: "Incidente: {change}",
    } satisfies Record<CaseEventType, string>,
    priority: {
      low: "Baixa",
      normal: "Normal",
      high: "Alta",
      urgent: "Urgente",
    } satisfies Record<CasePriority, string>,
    status: {
      new: "Novo",
      in_progress: "Em atendimento",
      waiting_customer: "Aguardando cliente",
      waiting_internal: "Aguardando equipe interna",
      resolved: "Resolvido",
      closed: "Encerrado",
    } satisfies Record<CaseStatus, string>,
  },
  status: {
    new: "Recebido",
    in_progress: "Em atendimento",
    waiting_customer: "Aguardando sua resposta",
    waiting_internal: "Em análise",
    resolved: "Resolvido",
    closed: "Encerrado",
  } satisfies Record<CaseStatus, string>,
  category: {
    deposits_withdrawals: "Depósitos e saques",
    operations: "Operações",
    account_verification: "Conta e verificação",
    bonuses_promotions: "Bônus e promoções",
    other: "Outro assunto",
    formal_complaint: "Reclamação formal",
  } satisfies Record<CaseCategory, string>,
} as const;

export type Dictionary = typeof ptBR;

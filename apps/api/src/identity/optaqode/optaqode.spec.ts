...
import { createJwtVerifier, signHs256 } from './optaqode-jwt.js';
...
const PROFILE = {
  id: 'PRF_01HZX8K2M4N6P8Q0R2S4T6V8W',
  user_code: '104233',
...
function fakeFetch(routes: Record<string, { status: number; body: unknown } | 'timeout'>): { fetch: FetchLike; calls: Array<{ url: string; headers: Record<string, string> }> } {
  ...
  const fetch: FetchLike = async (url, init) => {
    ...
    return { status: route.status, json: async () => route.body, text: async () => JSON.stringify(route.body) };
  };
  return { fetch, calls };
}

describe('optaqode configuration (PH-13, DEC-0045)', () => {
  it('needs the base URL, https outside localhost, and names the missing credential per adapter', () => {
    ...
    expect(() => requireIdentityConfig(config)).toThrow(/exactly one of ORBIT_JWT_SECRET/);
    expect(() => requireIdentityConfig(readOptaqodeConfig({ ...BASE, ORBIT_JWT_SECRET: 's', ORBIT_JWT_JWKS_URL: 'https://x/jwks' }))).toThrow(/exactly one/);
    expect(() => requireServiceConfig(config, 'records')).toThrow(/ORBIT_SERVICE_TOKEN is required for the optaqode records/);
  ...
  it('adapter selection: simulated by default, optaqode only with its material, unknown names refused', () => {
    ...
    expect(() => resolveIdentityProviderName({ SUPPORT_IDENTITY_PROVIDER: 'optaqode', ...BASE })).toThrow(/exactly one of/);
    expect(resolveIdentityProviderName({ SUPPORT_IDENTITY_PROVIDER: 'optaqode', ...BASE, ORBIT_JWT_SECRET: SECRET, NODE_ENV: 'production' })).toBe('optaqode');
    ...
    const real = { SUPPORT_ORBIT_RECORDS: 'optaqode', ...BASE, ORBIT_SERVICE_TOKEN: 'svc' };
...
describe('optaqode token verification', () => {
...
describe('optaqode identity provider', () => {
  ...
  it('resolves a customer from the bearer token, a staff member through the directory, and nobody otherwise', async () => {
    const identity = new OptaqodeIdentity(config, directory);
    const customer = signHs256({ sub: 'PRF_01HZX8K2M4N6P8Q0R2S4T6V8W', exp }, SECRET);
    await expect(identity.resolve({ authorization: `Bearer ${customer}` })).resolves.toEqual({ kind: 'customer', id: 'PRF_01HZX8K2M4N6P8Q0R2S4T6V8W', source: 'orbit' });
    const staff = signHs256({ person_id: 'PRF_staff', name: 'Carla Nunes', exp }, SECRET);
    await expect(identity.resolve({ authorization: `Bearer ${staff}`, 'x-orbit-actor': 'staff' })).resolves.toEqual({ kind: 'staff', id: 'PRF_staff', role: 'supervisor', displayName: 'Carla Nunes', source: 'orbit' });
    // A back-office user outside the support directory is nobody; a customer token cannot claim staff.
...
describe('optaqode mappers (shapes from the broker frontend)', () => {
  it('maps the profile to the masked customer summary and the verified contact address', () => {
    ...
    expect(summary).toEqual({
      userId: 'PRF_01HZX8K2M4N6P8Q0R2S4T6V8W',
      username: 'alice.souza',
  ...
  it('maps back-office roles to support roles (provisional, DEC-0045 c) and keeps only active members', () => {
    expect(toStaffRole('super_admin')).toBe('admin');
    expect(toStaffRole('finance_manager')).toBe('supervisor');
...
describe('optaqode records adapter', () => {
  ...
  it('reads the customer 360 with the service credential and answers available, not_found, timeout or unavailable honestly', async () => {
    const { fetch, calls } = fakeFetch({
      '/api/v1/admin/rastreio/PRF_01HZX8K2M4N6P8Q0R2S4T6V8W': { status: 200, body: RASTREIO },
    ...
    const summary = await records.customerSummary('PRF_01HZX8K2M4N6P8Q0R2S4T6V8W');
    ...
    const list = await records.listRecords('PRF_01HZX8K2M4N6P8Q0R2S4T6V8W');
    expect(list.state === 'available' && list.data.length).toBe(3);
    await expect(records.getRecord('PRF_01HZX8K2M4N6P8Q0R2S4T6V8W', 'withdrawal', 'wd_9f8e7d6c5b4a39281706')).resolves.toMatchObject({ state: 'available', data: { title: 'Saque' } });
    await expect(records.getRecord('PRF_01HZX8K2M4N6P8Q0R2S4T6V8W', 'withdrawal', 'dep_0a1b2c3d4e5f6a7b8c9d')).resolves.toMatchObject({ state: 'unavailable', reason: 'not_found' });
    await expect(records.contactEmail('PRF_01HZX8K2M4N6P8Q0R2S4T6V8W')).resolves.toMatchObject({ state: 'available', data: 'alice.souza@example.com' });
    ...
    expect(calls.some((c) => c.url.includes('PRF_missing'))).toBe(true);
  });
});
...
describe('optaqode staff directory', () => {
  ...
  it('lists active members across pages with mapped roles, caches the listing, and keeps the last good one on a failed refresh', async () => {
    let fail = false;
    const page1 = { items: [{ id: 'PRF_a', name: 'Ana', role: 'auditor', status: 'active' }, { id: 'PRF_c', name: 'Carla', role: 'finance_manager', status: 'active' }, { id: 'PRF_p', name: 'Paused', role: 'admin', status: 'paused' }], pagination: { page: 1, total_pages: 2 } };
    ...
    let clock = 0;
    const directory = new OptaqodeStaffDirectory(config, new OptaqodeClient(config, fetch), () => clock);

...
| Integration / CI / release | Candidate: the PH-4.1 commit on `main` (child of `85dc567`). Local: `npm run verify` exit 0, `npm run build` exit 0, browser smoke 33/33 — `docs/evidence/PH-4.1-verification.md` (CI verdict recorded there when known). Previous: `541753a` CI green (`docs/evidence/CYCLE-1-verification.md`). Release: none, not authorized. |
...
Preconditions: CI green on the PH-4.1 commit (`gh run list --limit 3`, record it in `docs/evidence/PH-4.1-verification.md`); tree clean; `npm run verify` passes.

...
Derived from: `docs/phases/ROADMAP.md`, `docs/phases/PH-9.md`, `docs/evidence/PH-9.2-verification.md`; base checkpoint: the PH-9.2 commit (after `41adb23`)
...
| Active phase / subphase | PH-9 `ACTIVE`; PH-9.1 `APPROVED` (case-domain debt), PH-9.2 `APPROVED` (staff workspace debt: BL-014, BL-013, BL-024); next PH-9.3 (customer panel: BL-010, BL-015) `PLANNED`, then PH-9.4 (operations, process, phase closure). |
...
| Integration / CI / release | Candidate: the PH-9.2 commit; CI pending at recording time (check `gh run list --limit 2`). `41adb23` (PH-9.1) CI run 34851816846 success (both jobs). Release: `v0.1.1-demo`, internal loopback demo; no production release authorized. |
...
## Next valid action
Action: plan and implement PH-9.3 — customer panel: BL-010 (attachments on the first message of a case) and BL-015 (the desktop "Fechar suporte" / "×" controls do nothing: hide the panel on desktop or remove them there).
Why now: PH-9.2 approved; PH-9.3 is the next planned block of the active phase.
Preconditions: the PH-9.2 commit green on both CI jobs.
Evidence/read first: `docs/phases/PH-9.md`, `docs/features/FEAT-CHAT/CONTEXT.md`, `docs/BACKLOG.md` rows BL-010, BL-015.

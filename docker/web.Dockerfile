...
# through this server's /api rewrite (API_ORIGIN), so only the web port is published.
...
EXPOSE 3000
HEALTHCHECK --interval=15s --timeout=5s --retries=5 CMD wget -qO- http://127.0.0.1:3000/ >/dev/null || exit 1
